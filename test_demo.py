...
def select_dataset(data_dir, mode):
    if mode == "test":
        ...
        path = [
            ...
            for p in sorted(
                glob.glob(os.path.join(data_dir, "LSDIR_DIV2K_test_HR/*.png"))
            )
        ]
    else:
        raise NotImplementedError(
            f"{mode} is not implemented in select_dataset"
        )
    return path
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    logger.info(
        "{:>16s} : {:<.3f} [M]".format("Max Memery", results[f"{mode}_memory"])
    )  # Memery
    logger.info(
...
def main(args):
    ...
    utils_logger.logger_info(
        "NTIRE2023-EfficientSR", log_path="NTIRE2023-EfficientSR.log"
    )
    ...
    for k, v in results.items():
        ...
        if args.include_test:
            ...
            ave_time = (
                f"{(v['valid_ave_runtime'] + v['test_ave_runtime']) / 2:3.2f}"
            )
            s += fmt.format(
                k,
                val_psnr,
            ...
            s += fmt.format(
                k, val_psnr, val_time, num_param, flops, acts, mem, conv
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hybrid_test",
        action="store_true",
        help="Hybrid test on DIV2K and LSDIR test set",
    )
